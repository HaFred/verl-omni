...
clear_pending_generate_provenance = traj.clear_pending_generate_provenance
build_generate_call_meta = traj.build_generate_call_meta
count_live_generate_artifacts_for_active_rollout = traj.count_live_generate_artifacts_for_active_rollout
get_pending_generate_provenance = traj.get_pending_generate_provenance
merge_pending_generate_provenance = traj.merge_pending_generate_provenance
...
set_pending_generate_provenance = traj.set_pending_generate_provenance
...
def test_generate_call_meta_tracks_initial_and_rewrite(tmp_path):
    relpath = "step_000001/sample_2.00"
    path_tokens = set_active_trajectory_relpath(relpath)
    prompt_token = set_active_user_prompt("a poster")
    png = tmp_path / "image_00_aaaaaaaaaaaa.png"
    png.write_bytes(b"\x89PNG\r\n\x1a\n")
    try:
        merge_pending_generate_provenance({"model_decode": "initial decode", "llm_prompt": "prompt v0"})
        initial = build_generate_call_meta(prompt="prompt v0", user_prompt="a poster")
        assert initial["call_role"] == "initial"
        ...
        reflection = "Reflection: rewrite the prompt"
        set_pending_generate_provenance({"reflection": reflection, "call_role": "rewrite"})
        rewritten = build_generate_call_meta(prompt="prompt v1", user_prompt="a poster")
        assert rewritten["call_role"] == "rewrite"
        assert rewritten["prev_tool_prompt"] == "prompt v0"
        assert rewritten["reflection"] == reflection
        assert rewritten["rewritten_prompt"] == "prompt v1"
        assert get_pending_generate_provenance() is None
    finally:
