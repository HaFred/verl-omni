...
class TestEdgeCases:
    ...
    def test_empty_turns(self):
        ...
        result = serialize_trajectories([traj], max_prompt_length=256,
...
class TestLogProbConsistency:
    ...
    def test_loss_mask_logprob_pairing(self):
        ...
        t0 = _make_turn(0, [10, 20, 30], [-0.1, -0.2, -0.3], "t0", "continue",
    ...
    def test_padding_positions_zero(self):
        ...
        if len(active) > 0:
            ...
            assert (logprobs[first_pad:] == 0.0).all(), \
                f"Padding positions [{first_pad}:] should be zero"
    ...
    def test_per_turn_length_equality(self):
        ...
        for turn in [t0, t1, t2]:
            assert len(turn.agent_tokens) == len(turn.agent_logprobs), \
                f"Turn {turn.turn_idx}: len(tokens)={len(turn.agent_tokens)} != len(logprobs)={len(turn.agent_logprobs)}"
...
class TestFreezeLogic:
    ...
    def test_freeze_prefix_matching(self):
        ...
        for name in ["gen_block.weight", "gen_block.bias",
                     "moe_gen_0.weight", "moe_gen_0.bias",
                     "moe_gen_1.weight", "moe_gen_1.bias"]:
...
class TestAgenticConfig:
    ...
    def test_enabled_override(self):
        ...
        c = AgenticConfig(enabled=True, max_turns=3, early_termination=False,
                          observation_token_length=64)
