...
def _last_assistant_text(messages: list[dict[str, Any]] | None) -> str:
    for message in reversed(messages or []):
        if message.get("role") == "assistant":
            return tool_message_text(message).strip()
    return ""
...
def _last_reflection_text(messages: list[dict[str, Any]] | None) -> str:
    for message in reversed(messages or []):
        if message.get("role") == "assistant":
            text = tool_message_text(message).strip()
            if re.search(r"\bReflection\s*:", text, re.IGNORECASE):
                return text
    return ""


def _parse_generate_prompt(tool_call: Any) -> str:
...
def _bind_generate_provenance(tool_call: Any, agent_data: AgentData) -> None:
    name = getattr(tool_call, "name", None) or (tool_call.get("name") if isinstance(tool_call, dict) else None)
    if name != "generate_image":
        return
    prompt = _parse_generate_prompt(tool_call)
    reflection = _last_reflection_text(agent_data.messages)
    updates: dict[str, Any] = {
        "model_decode": _last_assistant_text(agent_data.messages),
        "llm_prompt": prompt,
    }
    if reflection:
        updates.update(reflection=reflection, llm_reflection=reflection)
    if get_latest_generate_prompt_for_active_rollout() or reflection:
        updates.update(controlled_by_reflection=True, call_role="rewrite")
...
class ImageGenToolAgentLoop(ToolAgentLoop):
    ...
    async def _call_tool(self, tool_call, tools_kwargs, agent_data):
        ...
        _bind_generate_provenance(tool_call, agent_data)
        try:
    ...
    async def _handle_generating_state(
        ...
        with trace_state(
            "decode",
            state_lane_id=self._rl_insight_lane(),
