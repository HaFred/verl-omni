...
`AgenticRewardMetrics.MIX_KEYS`); absent keys are skipped:
...
and an explicit `+…agent_loop_manager_class=…OmniAgentLoopManager`
...
| Agent loop | `image_gen_tool_agent` + `OmniAgentLoopManager` (Hydra): force-first gen; forced Reflection after judge; YES/max-pass → policy `Done.` |
| Tools | `verl_omni/tools/image_gen.py`: `generate_image` + `judge_image` |
