...
#
# ENABLE_RM=1 scores images through the reward loop manager itself: the RM pool publishes
# an OpenAI-compatible router (``reward_router_address``), ``VisualRewardManager`` forwards
# it to ``bagel_rm_image_scorer``, and the pool's own model returns the C/A numbers and the
# Yes/No turn signals. No second model, no shared card. The frozen 9B sidecar is now an
# explicit opt-in (``JUDGE_SERVER=1``) for the case where the RM lane is off but a judge is
# still wanted; it is shadowed by the router whenever ENABLE_RM=1. ``JUDGE_URL`` reuses a
# sidecar you started yourself; ``JUDGE_ENV_FILE`` points at an operator env
# (e.g. ``~/fred/fred_verlomni_agentic_multiturn_pr1.sh``) to pick up ``JUDGE_IMAGE_MODEL``
# without sourcing it.
set -x
...
# and the failure surfaces early and misleadingly -- the *dataset* build imports
...
# Only applied when the shim is actually present, so a host with a native CUDA 13
# driver keeps its own library and this stays a no-op there.
...
# ---------------------------------------------------------------------------
# GEN reward source when ENABLE_RM=1: the reward loop manager's own model.
#
# ``bagel_rm_image_scorer``'s mid-loop branch forwards each PNG by HTTP to a VL judge
# (``agentic_image_judge_client.call_reflect_vlm`` -> ``post_vllm_chat`` ->
# ``{vllm_url}/v1/chat/completions``). With a reward model deployed, that URL no longer
# has to be an external process: ``RewardModelManager`` already stands up an
# OpenAI-compatible router in front of the RM replicas, ``RewardLoopManager`` publishes
# it as ``reward_router_address``, and ``VisualRewardManager`` forwards it into the
# scorer. The scorer prefers that router (see ``_resolve_judge_endpoint``), so the
# reward pool's own Qwen3.5 model produces both the C/A numbers and the Yes/No
# (``good_enough``) turn signals -- same prompt, same parse, one process fewer and no
# GPU shared with a second 9B judge.
#
# The frozen sidecar below is therefore an explicit opt-in fallback, not the default.
#
# Two knobs keep the reward pool's own boot from racing the colocated actor:
#
#  * ``max_model_len`` — unset it defaults to the RM's ``max_position_embeddings``
#    (262144), and vLLM then sizes KV for that against ``max_num_seqs``. A judge reads
#    one image plus a short prompt, so that ask is pure waste; pin it to the episode's
#    own window instead.
#  * ``kv_cache_memory_bytes`` — pins KV per GPU in bytes. Setting it takes vLLM's
#    ``determine_available_memory`` down an early-return path that SKIPS the
#    memory-profiling snapshot comparison entirely, which is the difference between a
#    deterministic boot and one that dies whenever a colocated component happens to
#    release memory mid-profile: the RM pool was lost to
#    ``AssertionError: Error in memory profiling. Initial free memory 73.34 GiB,
#    current free memory 74.27 GiB`` (bare ``assert``, no tolerance knob,
#    vLLM ``gpu_worker.py``). It also decouples RM KV from ``REWARD_GPU_MEM_UTIL``, so
#    the pool stops reserving a card fraction it mostly does not use — that reservation
#    is what OOMed the actor's initial LoRA-merge publish at 0.15 and what starved the
#    RM's own cache blocks at 0.08.
REWARD_MAX_MODEL_LEN=${REWARD_MAX_MODEL_LEN:-8192}
REWARD_KV_CACHE_BYTES=${REWARD_KV_CACHE_BYTES:-2147483648}  # 2 GiB per RM GPU
# It is only useful when the RM lane is OFF but a scorer still needs a judge, and it is
# shadowed by the router whenever ``ENABLE_RM=1``.
JUDGE_SERVER=${JUDGE_SERVER:-0}  # external judge is opt-in; the RM pool scores by default
JUDGE_URL=${JUDGE_URL:-http://127.0.0.1:8093}
# What the scorer is told to fall back to. Empty unless we own a sidecar: with the reward
# pool scoring internally, a stale/dead sidecar address must not survive in the knobs as a
# silent fallback (the router still wins, but an unreachable URL should not be reachable at
# all when the internal path is the intended one).
JUDGE_VLLM_URL=""
[[ "$JUDGE_SERVER" == "1" ]] && JUDGE_VLLM_URL="$JUDGE_URL"
...
if [[ "$JUDGE_SERVER" == "1" ]]; then
  ...
  # The internal path: no sidecar, but do not let a stale ``JUDGE_URL`` on disk mean
  # nothing -- if one happens to answer it is harmless (the router still wins when the
  # manager supplies one), and if none does that is now the expected state, not an error.
  if _judge_ready; then
    echo "[INFO] image judge detected at ${JUDGE_URL} but unused: the reward pool's router scores"
  else
    echo "[INFO] ENABLE_RM=1: image rewards and Yes/No signals come from the reward loop manager's"
    echo "[INFO]   own router (reward_router_address -> ${REWARD_MODEL:-<reward.reward_model.model_path>})"
  fi
fi
...
#     NOTE (fixed 2026-09-21): these two knobs used to be *dead for image generation*.
#     The agentic GEN tool read `actor_rollout_ref.rollout` (the training config) and
#     stamped it over the val overlay, so validation images still ran at the training
#     noise_level/GEN_STEPS. `build_gen_sampling_params` now lets the worker's per-phase
#     `base` win, so VAL_GEN_STEPS/VAL_NOISE_LEVEL reach the diffusion request; expect
#     validation PNGs to change from stochastic SDE to a deterministic 50-step ODE decode.
#     Separately, `sde_window_range` is now clamped to the real denoise-step count, so the
#     1-GPU branch's GEN_STEPS=4 (3 denoise steps vs a `[0, 7]` range) can no longer draw
#     a window past the whole schedule and silently record no noise/log-probs at all.
#
...
# step_000001. That is not hypothetical: on 2026-09-21 a fresh run overwrote
# ``bagel_corl_pr1``'s step_000001 while it was being read, so "step 1" and "step 2" in
# that directory came from two different runs and a within-run regression appeared where
# there was none.
#
# Binding the dump root under ``$RUN_DIR`` gives every run its own trace corpus, so
...
"${_PY}" -m verl_omni.trainer.main_omni \
    ...
    reward.reward_model.rollout.max_model_len=$REWARD_MAX_MODEL_LEN \
    +reward.reward_model.rollout.engine_kwargs.vllm.kv_cache_memory_bytes=$REWARD_KV_CACHE_BYTES \
